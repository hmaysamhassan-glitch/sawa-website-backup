...
function ArrivingArrow() {
  return (
    <motion.svg
      width="432" height="92" viewBox="0 0 432 92" fill="none"
      initial={{ opacity: 0 }}
      whileInView={{ opacity: 1 }}
      viewport={{ once: true }}
      transition={{ duration: 0.4 }}
      aria-hidden="true"
      style={{ display: "block", margin: "0 auto 40px", width: "clamp(300px,36vw,432px)", height: "auto" }}
    >
      {/* The journey path — sweeps in from left */}
      <motion.path
        d="M 4 60 C 84 60 110 22 188 28 C 266 34 294 66 372 52 C 396 48 410 38 424 31"
        ...
        opacity={0.85}
      />
      {/* Arrow head — filled triangle marking arrival at OWNED */}
      <motion.path
        d="M424 31 L409 45 L404 32 Z"
        fill={STONE}
        initial={{ opacity: 0 }}
        whileInView={{ opacity: 1 }}
        viewport={{ once: true }}
      ...
      />
      {/* Milestones — CHANNELS and AUDIENCE get dots; OWNED is the arrow tip */}
      {[
        { cx: 4,   cy: 60, label: "CHANNELS" },
        { cx: 188, cy: 28, label: "AUDIENCE" },
        { cx: 424, cy: 31, label: "OWNED"    },
      ].map(({ cx, cy, label }, i) => (
        <motion.g key={label}
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          ...
          {i !== 2 && <circle cx={cx} cy={cy} r={4} fill="rgba(143,193,227,0.55)" />}
          <text x={cx} y={i === 0 ? cy + 22 : cy - 15}
    ...
    >
      ...
      <motion.div
        ...
        style={{
          ...
          color: "transparent", WebkitTextStroke: "1.4px rgba(143,193,227,0.26)",
